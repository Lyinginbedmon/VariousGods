...
import com.lying.variousgods.api.event.LivingConsumableEvent.Drink;
import com.lying.variousgods.api.event.LivingConsumableEvent.Eat;
...
public class ContextBus
{
	...
	public static void onLivingDamage(LivingDamageEvent event)
	{
		...
		if(assailant != null && assailant.getType() == EntityType.PLAYER)
		{
			...
			if(victim.getType().is(VGEntityTags.BOSS))
	...
	public static void onLivingDeath(LivingDeathEvent event)
	{
		...
		if(victim.getType().is(VGEntityTags.BOSS) && assailant != null && assailant.getType() == EntityType.PLAYER)
		{
	...
	{
		PlayerData data = PlayerData.getCapability(player);
		if(item.is(VGItemTags.TABOO))
			data.addQuotient(ContextQuotients.EAT_TABOO.getId(), 1);
		
		for(TagKey<Item> tag : VGItemTags.DIET_TAGS)
			if(item.is(tag))
			{

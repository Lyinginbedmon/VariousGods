...
public class PathingMove
{
	...
	{
		MOVES.add(moveIn);
		VariousGods.LOG.info("# * Registered move: "+moveIn.getName());
	}
	...
	static
	{
		VariousGods.LOG.info("# Initialising Hearth Light pathfinder moveset");
		...
		addMove(new PathingMove("climb_up")
		{
			...
			public void addToPath(List<BlockPos> path)
			{
				path.add(path.get(path.size() - 1).above());
			}
		});
		...
		VariousGods.LOG.info("# "+MOVES.size()+" possible moves");
	}
}

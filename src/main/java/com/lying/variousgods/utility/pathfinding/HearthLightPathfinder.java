package com.lying.variousgods.utility.pathfinding;
...
public class HearthLightPathfinder
{
	...
	public boolean searchCompleted() { return this.currentSearch != null && this.currentSearch.finished(); }
	...
	public void tickPath(Level world)
	{
		...
		if(currentSearch.finished())
		{
//			VariousGods.LOG.info("Hearth Light pathfinder completed search at "+currentSearch.nodesSearched()+" nodes "+(currentSearch.successful() ? "SUCCESS" : "FAILED"));
	...
	{
//		VariousGods.LOG.info("Hearth Light pathfinder attempting path between "+position.toShortString()+" and "+destination.toShortString()+" (distance "+(int)Mth.sqrt((float)position.distSqr(destination))+")");
		start();

...
import com.lying.variousgods.api.event.MiracleEvent.CheckMiracleEvent;
import com.lying.variousgods.api.event.MiracleEvent.PerformMiracleEvent;
...
public abstract class Miracle
{
	...
	{
		Optional<Holder<Miracle>> holder = VGRegistries.MIRACLES_REGISTRY.get().getHolder(this);
	...
	public static boolean checkMiracle(Player playerIn, Miracle miracleIn)
	{
		...
		else
		{
			...
			{
				VariousGods.LOG.info("Miracle denied! No deity or deity does not have "+miracleIn.getRegistryName().getId());
				miracleEvent.setResult(Result.DENY);
			}
			else if(!data.canHaveMiracle())
			{
				VariousGods.LOG.info("Miracle denied! On cooldown");
				miracleEvent.setResult(Result.DENY);
			}
			else
			{
				...
				{
					VariousGods.LOG.info("Miracle denied! Opinion too low");
					miracleEvent.setResult(Result.DENY);
				}
				else if(opinion < god.getRandom().nextFloat())
				{
					VariousGods.LOG.info("Miracle denied! Bad luck");
					miracleEvent.setResult(Result.DENY);
				}
			}
		}
		
		VariousGods.EVENT_BUS.post(miracleEvent);
	...
	{
		VariousGods.LOG.info("Miracle performed! "+miracleIn.getRegistryName().getId());
		PlayerData data = PlayerData.getCapability(playerIn);
		...
		VariousGods.EVENT_BUS.post(new PerformMiracleEvent(playerIn, god, miracleIn));
	}
